...
remote_ref=""
remote_target_commit=""
local_target_commit=""

case "$REF" in
  origin/*)
    remote_ref="refs/heads/${REF#origin/}"
    ;;
  ...
  refs/remotes/origin/*)
    ...
    ;;
  *)
    if git -C "$STAGING_WORKTREE" rev-parse --verify "${REF}^{commit}" >/dev/null 2>&1; then
      local_target_commit="$(git -C "$STAGING_WORKTREE" rev-parse --verify "${REF}^{commit}")"
    else
...
if [ -n "$remote_ref" ]; then
  remote_target_commit="$(git -C "$STAGING_WORKTREE" ls-remote --exit-code origin "$remote_ref" | awk '{print $1}')" ||
    die "Target ref does not resolve on origin without fetching: $REF"
  [ -n "$remote_target_commit" ] || die "Target ref returned no commit from origin: $REF"
fi

target_commit="${remote_target_commit:-$local_target_commit}"
[ -n "$target_commit" ] || die "Target ref does not resolve to a commit: $REF"
...
Dry-run resolution used read-only commands only and did not fetch or update local Git refs.
