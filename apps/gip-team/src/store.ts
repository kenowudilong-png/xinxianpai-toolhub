...
function getAgentProfileValidationError(settings: AppSettings): { profile: ApiProfile | null; message: string } | null {
  ...
  if (normalized.agentApiConfigMode === 'hybrid') {
    ...
    if (!imageProfile) return { profile: null, message: '图像模型 API 配置不存在，请在助手配置页选择可用的图像模型配置。' }
...
async function storeTaskOutputImages(task: TaskRecord, images: string[], serverImageIds?: string[]) {
  ...
  try {
    for (const [index, dataUrl] of images.entries()) {
      ...
      const serverImageId = serverImageIds?.[index]
      const stored = serverImageId
        ? { id: await storeImageWithId(serverImageId, outputDataUrl, 'generated'), width: undefined, height: undefined }
        : await storeImageWithSize(outputDataUrl, 'generated')
...
export async function submitAgentMessage() {
  ...
  const galleryProfile = getActiveApiProfile(normalizedSettings)
  if (galleryProfile.provider !== 'openai' || galleryProfile.apiMode !== 'responses') {
...
async function executeTask(taskId: string) {
  ...
  try {
    ...
    // 存储输出图片
    const { outputIds, outputDataUrls, outputImageSizes, transparentOriginalImageIds } = await storeTaskOutputImages(task, result.images, result.imageIds)
