...
export async function ensureImageCached(id: string): Promise<string | undefined> {
  ...
  if (import.meta.env.MODE !== 'test') {
    const serverImage = await getServerImage(id).catch(() => undefined)
    if (serverImage) {
      cacheImage(id, serverImage)
      return serverImage
    }
  }
  return undefined
...
export async function ensureImageThumbnailCached(id: string): Promise<{ dataUrl: string; width?: number; height?: number } | undefined> {
  ...
  if (!rec?.thumbnailDataUrl) {
    if (import.meta.env.MODE !== 'test') {
      const serverImage = await getServerImage(id).catch(() => undefined)
      if (serverImage) {
        await storeImageWithId(id, serverImage, 'generated')
        const serverThumbnail = await getStoredFreshImageThumbnail(id)
        if (serverThumbnail?.thumbnailDataUrl) {
          const thumbnail = {
            dataUrl: serverThumbnail.thumbnailDataUrl,
            width: serverThumbnail.width,
            height: serverThumbnail.height,
            thumbnailVersion: serverThumbnail.thumbnailVersion,
          }
...
function startThumbnailBackfill(id: string) {
  ...
  void (async () => {
    ...
    if (!image && import.meta.env.MODE !== 'test') {
      const serverImage = await getServerImage(id).catch(() => undefined)
      if (serverImage) {
        await storeImageWithId(id, serverImage, 'generated')
  ...
  persist(
    (set, get) => ({
      ...
      setSettings: (s) => set((st) => {
        ...
        if (import.meta.env.MODE !== 'test' && (window.location.pathname.startsWith('/admin') || window.location.pathname.startsWith('/tools/gip/app/admin')) && new URLSearchParams(window.location.search).get('settings') === '1') {
          void teamApi.putSettings(settings).catch(() => undefined)
        }
...
async function flushAgentConversationsToIndexedDB() {
  ...
  try {
    do {
      ...
      if (import.meta.env.MODE !== 'test') {
        await syncServerAgentConversations(conversations).catch(() => undefined)
      }
...
export async function initStore() {
  ...
  const serverTasks = import.meta.env.MODE !== 'test' ? await getServerTasks().catch(() => null) : null
  const serverAgentConversations = import.meta.env.MODE !== 'test' ? await getServerAgentConversations().catch(() => null) : null
  const storedTasks = serverTasks ?? await getAllTasks()
  const storedAgentConversations = normalizeAgentConversations(serverAgentConversations ?? await getAllAgentConversations())
...
export async function submitTask(options: { allowFullMask?: boolean; useCurrentApiProfileWhenReusedMissing?: boolean } = {}) {
  ...
  if (import.meta.env.MODE === 'test' && validateApiProfile(activeProfile)) {
...
async function createAgentUserInputItem(conversation: AgentConversation, round: AgentRound, message: AgentMessage, tasks: TaskRecord[]) {
  ...
  return {
    ...
    content: [
      ...
      ...imageDataUrls.map((dataUrl) => ({ type: 'input_image', image_url: dataUrl, detail: 'auto' })),
    ],
  }
}

async function createAgentGeneratedImagesInputItem(round: AgentRound, tasks: TaskRecord[]) {
  const contentParts: Array<{ type: string; text?: string; image_url?: string; detail?: string }> = []
  let imageIndex = 0
    ...
    const executeBatchFunctionCall = async (functionCallItem: ResponsesOutputItem): Promise<string> => {
      ...
      const batchPromises = batchExecutionItems.map(async ({ item, batchToolCallId, references, referenceIds }) => {
        ...
        try {
          ...
          return { batchItemId: item.id, image: null, error: message } satisfies BatchImageCallResult
        }
      })
...
export async function removeMultipleTasks(taskIds: string[]) {
  ...
  for (const id of taskIds) {
    ...
    if (import.meta.env.MODE !== 'test') await deleteServerTask(id).catch(() => undefined)
  }
...
export async function removeTask(task: TaskRecord) {
  ...
  if (import.meta.env.MODE !== 'test') await deleteServerTask(task.id).catch(() => undefined)
...
export async function exportData(options: ExportOptions = { exportConfig: true, exportTasks: true }) {
  try {
    if (import.meta.env.MODE !== 'test' && options.exportTasks && !options.exportConfig) {
      await exportServerTasks()
      useStore.getState().showToast('服务端数据已导出', 'success')
      return
    }
...
export async function createInputImageFromFile(file: File): Promise<InputImage | null> {
  ...
  const upload = import.meta.env.MODE !== 'test' ? await uploadServerImage(file).catch(() => null) : null
  const id = upload?.id || await storeImage(dataUrl, 'upload')
  cacheImage(id, dataUrl)
  return { id, dataUrl }
}
