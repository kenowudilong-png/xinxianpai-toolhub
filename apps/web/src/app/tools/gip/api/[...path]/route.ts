...
const gipProxyDispatcher = new Agent({ headersTimeout: 30 * 60 * 1000, bodyTimeout: 30 * 60 * 1000 });
...
async function proxy(request: NextRequest, path: string[], mode: "api" | "app") {
  ...
  const upstream = await fetch(upstreamUrl, { method, headers, body, redirect: "manual", dispatcher: gipProxyDispatcher } as RequestInit & { dispatcher: Agent });
