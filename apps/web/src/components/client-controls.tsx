...
export function Sidebar({ tools, isAdmin }: { tools: ToolRow[]; isAdmin: boolean }) { const pathname = usePathname(); const [collapsed, setCollapsed] = useState(false); return <aside className={`${collapsed ? "w-16" : "w-64"} flex flex-col gap-2 border-r bg-card/80 p-3 backdrop-blur transition-all`}><a className={navCls(pathname === "/")} href="/"><Home className="size-4"/><span className={collapsed ? "hidden" : ""}>首页</span></a>{tools.map(t=><a key={t.id} className={navCls(pathname.startsWith(t.path))} href={t.path}><span>{t.icon}</span><span className={collapsed ? "hidden" : ""}>{t.name}</span></a>)}<div className="my-2 border-t" />{isAdmin && <a className={navCls(pathname.startsWith("/admin"))} href="/admin/users"><Settings className="size-4"/><span className={collapsed ? "hidden" : ""}>管理</span></a>}<button className={navCls(false)+" mt-auto"} onClick={()=>setCollapsed(!collapsed)} type="button">{collapsed ? <ChevronsRight className="size-4"/> : <ChevronsLeft className="size-4"/>}<span className={collapsed ? "hidden" : ""}>收起</span></button></aside> }
...
export function ToolToolbar({path}:{path:string}){const r=useRouter();return <div className="flex gap-2"><Button variant="outline" onClick={()=>r.refresh()}><RefreshCw className="size-4"/>刷新</Button><Button variant="outline" asChild><a href={path} target="_blank"><ExternalLink className="size-4"/>新标签</a></Button><Button variant="outline" asChild><a href="/"><ChevronLeft className="size-4"/>返回</a></Button></div>}
