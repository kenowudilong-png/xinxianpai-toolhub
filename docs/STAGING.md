...
Dry-run mode prints the current staging branch, current staging commit, target ref, target commit, and the Git commands that would run in execute mode.

Dry-run mode resolves remote branch refs with read-only remote inspection and does not run `git fetch`, update local Git refs, update the staging worktree, start services, read secrets, run migrations, or touch production.
